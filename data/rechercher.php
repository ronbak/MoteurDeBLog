...
if(isset($_POST['search'])){
    ...
    $search = explode(" ",mysql_real_escape_string($_POST['search'])); //décompose la recherche



    $i=0;
    $req = "SELECT * FROM articles WHERE contenu LIKE '%$search[$i]%'  "; //prépare larequete
    $like="";
    while($i<count($search)){
        $like.=" OR contenu LIKE '%search[$i+1]%' "; //ajoute tout les mots de la recherche
        $i++;
    }
    ...
    if (mysql_num_rows($res) > 0) {
        ...
        while($data = mysql_fetch_array($res)) //affiche tous les résultats
        {
